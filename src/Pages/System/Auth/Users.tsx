...
import { useAuthRoles, useAuthUser } from '../../../utils/fetchUrls';
...
import { AuthRoleType, authUser, AuthUserType } from '../../../types/System/Auth/auth';
...
const fields: ModalFieldConfig[] = [
    ...
    { name: "roleId", label: "角色", type: "select", options: [], validation: ["isEmpty"] },
    { name: "name", label: "使用者全名", type: "text", validation: ["isEmpty"] },
    { name: "type", label: "使用者類型", type: "select", options: [{ value: "0", label: "員工" }, { value: "1", label: "代送商" }] },
...
const Users = () => {
    ...
    const roleApi = useAuthRoles<AuthRoleType>();
    ...
    const fieldsCheck = async () => {
        const newFields = await Promise.all(fields.map<Promise<ModalFieldConfig>>(async (field) => {
            if (field.name === 'roleId') {
                ...
                if (result.success) {
                    const temp = { ...field };
                    temp.options = (result.data as AuthRoleType[]).map((data) => ({ value: data.id, label: data.name }));
                    return temp;
                }
            }
            return field;
